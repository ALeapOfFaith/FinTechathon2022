...
# Preprocessing
DEFINE_bool("do_prepro", default=False,
      help="Perform preprocessing only.")
DEFINE_integer("num_proc", default=1,
      help="Number of preprocessing processes.")
DEFINE_integer("proc_id", default=0,
      help="Process id for preprocessing.")

# Model
DEFINE_string("model_config_path", default=None,
      help="Model config path.")
DEFINE_float("dropout", default=0.1,
      help="Dropout rate.")
DEFINE_float("dropatt", default=0.1,
      help="Attention dropout rate.")
DEFINE_integer("clamp_len", default=-1,
      help="Clamp length.")
DEFINE_string("summary_type", default="last",
      help="Method used to summarize a sequence into a vector.")
DEFINE_bool("use_bfloat16", default=False,
      help="Whether to use bfloat16.")

# Parameter initialization
DEFINE_enum("init", default="normal",
                  enum_values=["normal", "uniform"],
                  help="Initialization method.")
DEFINE_float("init_std", default=0.02,
                   help="Initialization std when init is normal.")
DEFINE_float("init_range", default=0.1,
                   help="Initialization std when init is uniform.")

# I/O paths
DEFINE_bool("overwrite_data", default=False,
                  help="If False, will use cached data if available.")
DEFINE_string("init_checkpoint", default=None,
                    help="checkpoint path for initializing the model. "
                    "Could be a pretrained model or a finetuned model.")
DEFINE_bool("init_global_vars", default=False,
                  help="If true, init all global vars. If false, init "
                  "trainable vars only.")
DEFINE_string("output", default="",
                    help="Output dir for TF records.")
DEFINE_string("predict", default="",
                    help="Dir for predictions.")
DEFINE_string("model_file", default="",
                    help="Sentence Piece model path.")
DEFINE_string("model", default="",
                    help="Directory for saving the finetuned model.")
DEFINE_string("train_file", default="",
                    help="Path of train file.")
DEFINE_string("predict_file", default="",
                    help="Path of prediction file.")

# Data preprocessing config
DEFINE_integer("max_seq_length",
                     default=512, help="Max sequence length")
DEFINE_integer("max_query_length",
                     default=64, help="Max query length")
DEFINE_integer("doc_stride",
                     default=128, help="Doc stride")
DEFINE_integer("max_answer_length",
                     default=64, help="Max answer length")
DEFINE_bool("uncased", default=False, help="Use uncased data.")

# TPUs and machines
DEFINE_bool("use_tpu", default=False, help="whether to use TPU.")
DEFINE_integer("num_hosts", default=1, help="How many TPU hosts.")
DEFINE_integer("num_core_per_host", default=8,
      help="8 for TPU v2 and v3-8, 16 for larger TPU v3 pod. In the context "
      "of GPU training, it refers to the number of GPUs used.")
DEFINE_string("tpu_job_name", default=None, help="TPU worker job name.")
DEFINE_string("tpu", default=None, help="TPU name.")
DEFINE_string("tpu_zone", default=None, help="TPU zone.")
DEFINE_string("gcp_project", default=None, help="gcp project.")
DEFINE_string("master", default=None, help="master")
DEFINE_integer("iterations", default=1000,
                     help="number of iterations per TPU training loop.")

# Training
DEFINE_bool("do_train", default=True, help="whether to do training")
DEFINE_integer("train_batch_size", default=48,
                     help="batch size for training")
DEFINE_integer("train_steps", default=8000,
                     help="Number of training steps")
DEFINE_integer("warmup_steps", default=0, help="number of warmup steps")
DEFINE_integer("save_steps", default=None,
                     help="Save the model for every save_steps. "
                     "If None, not to save any model.")
DEFINE_integer("max_save", default=5,
                     help="Max number of checkpoints to save. "
                     "Use 0 to save all.")
DEFINE_integer("shuffle_buffer", default=2048,
                     help="Buffer size used for shuffle.")

# Optimization
DEFINE_float("learning_rate", default=3e-5, help="initial learning rate")
DEFINE_float("min_lr_ratio", default=0.0,
                   help="min lr ratio for cos decay.")
DEFINE_float("clip", default=1.0, help="Gradient clipping")
DEFINE_float("weight_decay", default=0.00, help="Weight decay rate")
DEFINE_float("adam_epsilon", default=1e-6, help="Adam epsilon")
DEFINE_string("decay_method", default="poly", help="poly or cos")
DEFINE_float("lr_layer_decay_rate", default=0.75,
                   help="Top layer: lr[L] = learning_rate."
                   "Lower layers: lr[l-1] = lr[l] * lr_layer_decay_rate.")

# / Prediction
DEFINE_bool("do_predict", default=False, help="whether to do predict")
DEFINE_integer("predict_batch_size", default=32,
                     help="batch size for prediction")
DEFINE_integer("n_best_size", default=5,
                     help="n best size for predictions")
DEFINE_integer("start_n_top", default=5, help="Beam size for span start.")
DEFINE_integer("end_n_top", default=5, help="Beam size for span end.")
DEFINE_string("target_key", default="best_f1",
...
def convert_examples_to_features(examples, sp_model, max_seq_length,
  ...
  for (example_index, example) in enumerate(examples):
    ...
    tok_cat_text = ''.join(para_tokens).replace(UNDERLINE, ' ')
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    # and the ation script will search for the best threshold
...
def input_fn_builder(input_glob, seq_length, is_training, drop_remainder,
  ...
  def input_fn(params):
    ...
    if len(input_paths) == 1:
      ...
      # For  we want no shuffling and parallel reading doesn't matter.
      if is_training:
...
def preprocess():
  ...
  train_rec_file = os.path.join(
      ...
      "{}.{}.slen-{}.qlen-{}.train.tf_record".format(
          basename, proc_id, max_seq_length,
          max_query_length))
...
def main(_):
  ...
  if save_steps is not None:
    iterations = min(iterations, save_steps)

  if not do_train and not do_predict:
    raise ValueError(
        "At least one of `do_train` and `do_predict` must be True.")

  if do_predict and not tf.gfile.Exists(predict):
    tf.gfile.MakeDirs(predict)
  ...
  if do_predict:
    examples = read_squad_examples(predict_file, is_training=False)

    with tf.gfile.Open(predict_file) as f:
      orig_data = json.load(f)["data"]

    rec_file = os.path.join(
        output,
        "{}.slen-{}.qlen-{}.tf_record".format(
            basename, max_seq_length, max_query_length))
    feature_file = os.path.join(
        output,
        "{}.slen-{}.qlen-{}.features.pkl".format(
            basename, max_seq_length, max_query_length))

    if tf.gfile.Exists(rec_file) and tf.gfile.Exists(
        feature_file) and not overwrite_data:
      tf.logging.info("Loading features from {}".format(feature_file))
      with tf.gfile.Open(feature_file, 'rb') as fin:
        features = pickle.load(fin)
    else:
      writer = FeatureWriter(filename=rec_file, is_training=False)
      features = []
      ...
      with tf.gfile.Open(feature_file, 'wb') as fout:
    ...
    output_nbest_file = os.path.join(
        predict, "nbest_predictions.json")
    output_null_log_odds_file = os.path.join(
        predict, "null_odds.json")

    ret = write_predictions(examples, features, cur_results,
