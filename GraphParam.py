...
class GraphParam(BaseParam):
    ...
    def check(self):
        ...
        if self.penalty is None:
            ...
            raise ValueError(
                "graph_param's penalty {} not supported, should be str type".format(self.penalty))
        else:
            self.penalty = self.penalty.upper()
            if self.penalty not in [consts.L1_PENALTY, consts.L2_PENALTY, 'NONE']:
                raise ValueError(
                    "graph_param's penalty not supported, penalty should be 'L1', 'L2' or 'none'")

        if not isinstance(self.tol, (int, float)):
            raise ValueError(
                "graph_param's tol {} not supported, should be float type".format(self.tol))

        if type(self.alpha).__name__ not in ["float", 'int']:
            raise ValueError(
                "graph_param's alpha {} not supported, should be float or int type".format(self.alpha))

        if type(self.optimizer).__name__ != "str":
            raise ValueError(
                "graph_param's optimizer {} not supported, should be str type".format(self.optimizer))
        else:
            self.optimizer = self.optimizer.lower()
            if self.optimizer not in ['sgd', 'rmsprop', 'adam', 'adagrad', 'nesterov_momentum_sgd', 'sqn']:
                raise ValueError(
                    "graph_param's optimizer not supported, optimizer should be"
        ...
        if not isinstance(self.learning_rate, (float, int)):
            raise ValueError(
                "graph_param's learning_rate {} not supported, should be float or int type".format(
                    self.learning_rate))
        ...
        if type(self.max_iter).__name__ != "int":
            raise ValueError(
                "graph_param's max_iter {} not supported, should be int type".format(self.max_iter))
        elif self.max_iter <= 0:
            raise ValueError(
                "graph_param's max_iter must be greater or equal to 1")

        if type(self.early_stop).__name__ != "str":
            raise ValueError(
                "graph_param's early_stop {} not supported, should be str type".format(
                    self.early_stop))
        else:
            self.early_stop = self.early_stop.lower()
            if self.early_stop not in ['diff', 'abs', 'weight_diff']:
                raise ValueError(
                    "graph_param's early_stop not supported, converge_func should be"
        ...
        if self.encrypt_param.method not in [consts.PAILLIER, None]:
            raise ValueError(
                "graph_param's encrypted method support 'Paillier' or None only")

        if type(self.decay).__name__ not in ["int", 'float']:
            raise ValueError(
                "graph_param's decay {} not supported, should be 'int' or 'float'".format(
                    self.decay))

        if type(self.decay_sqrt).__name__ not in ['bool']:
            raise ValueError(
                "graph_param's decay_sqrt {} not supported, should be 'bool'".format(
                    self.decay_sqrt))
...
class BaseGraphParam(LogisticParam):
    """
    Parameters
    ----------
    ...
    def check(self):
        ...
        if type(self.re_encrypt_batches).__name__ != "int":
            raise ValueError(
                "graph_param's re_encrypt_batches {} not supported, should be int type".format(
                    self.re_encrypt_batches))
        elif self.re_encrypt_batches < 0:
            raise ValueError(
                "graph_param's re_encrypt_batches must be greater or equal to 0")

        if not isinstance(self.aggregate_iters, int):
            raise ValueError(
                "graph_param's aggregate_iters {} not supported, should be int type".format(
